...
public class SemanticClusteredMetadataManagerReporter implements ClusteredMetadataManagerReporter {
    ...
    public SemanticClusteredMetadataManagerReporter(SemanticMetricRegistry registry) {
        ...
        this.search = new SemanticFutureReporter(registry, id.tagged("what", "search", "unit", Units.READ));
        this.tagKeySuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-key-suggest", "unit",
                Units.READ));
        this.tagSuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-suggest", "unit", Units.READ));
        this.count = new SemanticFutureReporter(registry, id.tagged("what", "count", "unit", Units.READ));
        this.keySuggest = new SemanticFutureReporter(registry, id.tagged("what", "key-suggest", "unit", Units.READ));
        this.tagValuesSuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-values-suggest", "unit",
                Units.READ));
        this.tagValueSuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-value-suggest", "unit",
                Units.READ));
    }
    ...
    public void registerShards(Set<Map<String, String>> shards) {
        ...
        for (final Map<String, String> shard : shards) {
            ...
            final FutureReporter findTags = new SemanticFutureReporter(registry, this.findTagsShard.tagged("shard",
                    shardName));
            final FutureReporter findKeys = new SemanticFutureReporter(registry, this.findKeysShard.tagged("shard",
                    shardName));
            final FutureReporter findSeries = new SemanticFutureReporter(registry, this.findSeriesShard.tagged("shard",
                    shardName));

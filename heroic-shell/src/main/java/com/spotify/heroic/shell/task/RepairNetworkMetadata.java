...
public class RepairNetworkMetadata implements CoreBridge.Task {
    public static void main(String argv[]) throws Exception {
        CoreBridge.standalone(argv, RepairNetworkMetadata.class);
    }
    ...
    public AsyncFuture<Void> run(final PrintWriter out, BaseParams base) throws Exception {
        ...
        for (final String npu : ImmutableList.of("a1", "a2", "b1", "b2")) {
            for (final String dest : ImmutableList.of("sto", "sjc", "lon", "ash")) {
                final Series series = new Series("network", ImmutableMap.<String, String> builder().put("site", "ash")
                        .put("npu", npu).put("what", "packet losses").put("component", "ipsec")
                        .put("role", "nwmonitor").put("provider", "ntt").put("from", "ash").put("env", "production")
                        .put("destination", dest).put("pod", "ash2").put("host", "ash2-nwmonitor-a1.ash2.spotify.net")
                        .put("hwid", "hw6380").build());

                futures.add(group.write(series, filter.getRange()));
            }
        }

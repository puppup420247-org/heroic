...
public class SemanticMetadataManagerReporter implements LocalMetadataManagerReporter {
    ...
    public SemanticMetadataManagerReporter(SemanticMetricRegistry registry) {
        ...
        findTimeSeries = new SemanticFutureReporter(registry, id.tagged("what", "find-time-series", "unit",
                Units.LOOKUP));
        countSeries = new SemanticFutureReporter(registry, id.tagged("what", "count-series", "unit", Units.LOOKUP));
        findKeys = new SemanticFutureReporter(registry, id.tagged("what", "find-keys", "unit", Units.LOOKUP));
        tagKeySuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-key-suggest", "unit", Units.LOOKUP));
        tagSuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-suggest", "unit", Units.LOOKUP));
        keySuggest = new SemanticFutureReporter(registry, id.tagged("what", "key-suggest", "unit", Units.LOOKUP));
        tagValuesSuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-values-suggest", "unit",
                Units.LOOKUP));
        tagValueSuggest = new SemanticFutureReporter(registry, id.tagged("what", "tag-value-suggest", "unit",
                Units.LOOKUP));
    }
